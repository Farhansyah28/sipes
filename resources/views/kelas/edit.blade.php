                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('kelas.update', $kela->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" name="tingkat" type="text" value="{{ old('tingkat', $kela->tingkat) }}" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Nama Kelas</label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" name="nama" type="text" value="{{ old('nama', $kela->nama) }}" required>
                        </div>
                        <div class="mb-6">
                            ...
                            <select name="wali_kelas_id" class="shadow border rounded w-full py-2 px-3 text-gray-700">
                                ...
                                @foreach($ustadzs as $u)
                                    <option value="{{ $u->id }}" {{ old('wali_kelas_id', $kela->wali_kelas_id) == $u->id ? 'selected' : '' }}>{{ $u->nama_lengkap }}</option>
                                @endforeach
                            </select>
                        </div>
