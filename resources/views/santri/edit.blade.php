                    ...
                    <form action="{{ route('santri.update', $santri->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" name="nis" type="text" value="{{ old('nis', $santri->nis) }}">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">NISN</label>
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" name="nisn" type="text" value="{{ old('nisn', $santri->nisn) }}">
                            </div>
                        </div>

                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Nama Lengkap</label>
                            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" name="nama_lengkap" type="text" value="{{ old('nama_lengkap', $santri->nama_lengkap) }}" required>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" name="tempat_lahir" type="text" value="{{ old('tempat_lahir', $santri->tempat_lahir) }}">
                            </div>
                            <div>
                                <label class="block text-gray-700 text-sm font-bold mb-2">Tanggal Lahir</label>
                                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" name="tanggal_lahir" type="date" value="{{ old('tanggal_lahir', $santri->tanggal_lahir) }}">
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <select name="orang_tua_id" class="shadow border rounded w-full py-2 px-3 text-gray-700">
                                    ...
                                    @foreach($orang_tuas as $ot)
                                        <option value="{{ $ot->id }}" {{ old('orang_tua_id', $santri->orang_tua_id) == $ot->id ? 'selected' : '' }}>{{ $ot->nama_ayah ?? $ot->nama_ibu }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="status" class="shadow border rounded w-full py-2 px-3 text-gray-700" required>
                                    @foreach(['Draft', 'Verifikasi', 'Tes', 'Lulus', 'Tidak Lulus', 'Daftar Ulang', 'Aktif', 'Alumni', 'Keluar'] as $stat)
                                        <option value="{{ $stat }}" {{ old('status', $santri->status) == $stat ? 'selected' : '' }}>{{ $stat }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <select name="kelas_id" class="shadow border rounded w-full py-2 px-3 text-gray-700">
                                    ...
                                    @foreach($kelas as $k)
                                        <option value="{{ $k->id }}" {{ old('kelas_id', $santri->kelas_id) == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
