            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('tahun_ajaran.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            + Tambah Data
                        </a>
                    </div>
                    ...
                    <table class="min-w-full bg-white border border-gray-200">
                        ...
                        <tbody>
                            @foreach ($tahun_ajarans as $index => $item)
                            <tr>
                                <td class="py-2 px-4 border-b">{{ $tahun_ajarans->firstItem() + $index }}</td>
                                ...
                                <td class="py-2 px-4 border-b">
                                    <a href="{{ route('tahun_ajaran.edit', $item->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-2">Edit</a>
                                    <form action="{{ route('tahun_ajaran.destroy', $item->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
